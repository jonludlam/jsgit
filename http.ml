...
module E = struct
  ...
  type t = {uri: Uri.t; headers: Cohttp.Header.t}
...
module Client = struct
  ...
  let call ?headers ?body meth e =
    ...
    Cohttp_lwt_xhr.Client.call ~headers ?body ~chunked:false meth uri
    >>= fun ((resp, _) as v) ->
    if
    ...
    then (
      ...
      Log.debug (fun l ->
          l ~header:"call" "Redirection from %a to %a." Uri.pp_hum uri
            Uri.pp_hum uri' ) ;
      Cohttp_lwt_xhr.Client.call ~headers ~body:(`Stream body_if_redirection)
